...
public class MangaPosterRowVertical extends MangaList {

	
	public MangaPosterRowVertical(List<MangaInfo> entries, File metadir, Predicate<File> dirFilter, Rectangle pane, boolean loadMetaFromDir, int selected) {
		
		this(entries, metadir, dirFilter, pane, loadMetaFromDir, false, selected);
	}
	
	public MangaPosterRowVertical(List<MangaInfo> entries, File metadir, Predicate<File> dirFilter, Rectangle pane, boolean loadMetaFromDir, boolean drawPanels, int selected) {
		super(entries, metadir, dirFilter, pane, MenuList.MODE_VERTICAL, loadMetaFromDir, drawPanels, selected);
		
		super.padV = 25f * displayScale;
		super.entryHeight = POSTER_HEIGHT * 0.75f * displayScale;
		super.drawSplitter = false;
	}
	...
	public void renderEntry(Graphics g, MangaInfo entry, float x, float y, boolean selected, int ind) {
		...
		y -= (height - (height / sc)) * 0.5f;
		
		if (!selected) {
			if (ind < super.selected)
				y -= height * 0.15f * 0.5f;
			else
				y += height * 0.15f * 0.5f;
		}
		...
		if (selected) {
			...
			// will be replaced anytime soon(tm)
			...
			x += width + 20f * displayScale;
			float y1 = y + (height - g.getFont().getHeight(s)) * 0f;
