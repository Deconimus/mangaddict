...
public class InputPanel extends Component {
	...
	public int mode;
	public boolean censor, ctrlPressed;
	...
	public void handleInput(int key, char c, boolean pressed) {
		...
		if (pressed) {
			...
			if (ctrlPressed && key == Input.KEY_V) {
				
				String cbData = null;
				
				try {
					cbData = Toolkit.getDefaultToolkit().getSystemClipboard().getData(DataFlavor.stringFlavor).toString().trim();
				} catch (Exception e) { e.printStackTrace(); }
				
				if (cbData != null) {
					
					if (Math.abs(selStart-selEnd) > 0) { deleteSelection(); }
					
					txt = txt.substring(0, cursor)+cbData+txt.substring(cursor);
					cursor = Math.min(cursor+cbData.length(), txt.length());
					cam = Math.max(Math.min(cam+cbData.length(), txt.length()-charsPerLine), 0);
