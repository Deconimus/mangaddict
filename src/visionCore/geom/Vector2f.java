...
public strictfp class Vector2f implements Serializable {
	...
	public void setTheta(double theta) {
		...
		float len = length();
		x = len * (float) FastMath.cos(StrictMath.toRadians(theta));
		y = len * (float) FastMath.sin(StrictMath.toRadians(theta));
...
//				* (float) FastTrig.sin(StrictMath.toRadians(theta));
	 ...
	 * 
	 * @param v The vector to add
	 * @param scale The scale which is applied to v at addition
	 * @return This vector - useful for chaning operations
	 */
	...
	public static float getAngle(Vector2f source, Vector2f end) {
		
		return (float)Math.atan2(end.y - source.y, end.x - source.x);
	}
	...
	public static Vector2f parse(String str) {
		...
		if (!str.startsWith("[Vector2f")) { return null; }
		...
		try {
			s = str.substring(str.indexOf("x")+2, str.indexOf(";", str.indexOf("x")));
			x = (float)(Double.parseDouble(s));
		} catch (Exception | Error e) { e.printStackTrace(); }
		
		try {
			s = str.substring(str.indexOf("y")+2, str.indexOf(";", str.indexOf("y")));
			y = (float)(Double.parseDouble(s));
		} catch (Exception | Error e) { e.printStackTrace(); }
